...
const createDoctorDepartmentService = async ({ doctorId, departmentId, role, joinedAt }) => {
  ...
  const existing = await DoctorDepartment.findOne({ where: { doctorId, departmentId } });
  if (existing) {
    throw { status: 400, message: 'Doctor is already assigned to this department' };
  }
